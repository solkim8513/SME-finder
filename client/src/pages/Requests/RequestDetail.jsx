...
export default function RequestDetail() {
  ...
  async function submitLog(e) {
    ...
    if (!message) return toast.error('Enter an update for the team');

    setSavingLog(true);
    try {
      const res = await api.post(`/sme-requests/${id}/logs`, { message });
      setReq(current => ({ ...current, logs: [res.data, ...(current?.logs || [])] }));
      setLogMessage('');
      toast.success('Team log updated');
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Created by {req.creator_first} {req.creator_last} · {new Date(req.created_at).toLocaleDateString()}
          </p>
        </div>
        ...
        <div className="card space-y-3">
          ...
          {req.assigned_sme_id ? (
            <>
              <Row
                label="Name"
                value={<Link to={`/smes/${req.assigned_sme_id}`} className="text-blue-600 hover:underline">{req.sme_name}</Link>}
              />
      ...
      <div className="card">
        <h3 className="font-semibold text-gray-800 mb-3">Team Log</h3>
        <form onSubmit={submitLog} className="space-y-3 mb-4">
          <textarea
            className="input"
            rows={3}
            placeholder="Add an update so the team can see what is going on..."
            value={logMessage}
            onChange={e => setLogMessage(e.target.value)}
          />
          <div className="flex justify-end">
            <button type="submit" className="btn-primary" disabled={savingLog || !logMessage.trim()}>
              {savingLog ? 'Saving...' : 'Add Update'}
            </button>
          </div>
        </form>

        {req.logs?.length === 0 ? (
          <p className="text-sm text-gray-400">No team updates yet.</p>
        ) : (
            ...
            {req.logs.map(log => (
              <div
                ...
                className={`rounded-lg border px-3 py-3 ${
                  log.entry_type === 'system' ? 'bg-blue-50 border-blue-100' : 'bg-gray-50 border-gray-100'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-medium text-gray-800">
                      ...
                      <span className="ml-2 text-xs uppercase tracking-wide text-gray-400">{log.entry_type}</span>
                    </div>
                    <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap">{log.message}</p>
                  </div>
        ...
        {req.notifications?.length === 0 ? (
          ...
          <div className="space-y-2">
            {req.notifications.map((n, i) => (
              <div
                key={`${n.type}-${n.sent_at}-${i}`}
                className={`flex items-start gap-3 text-sm p-2 rounded-md ${n.status === 'failed' ? 'bg-red-50' : 'bg-gray-50'}`}
              >
                <span className="text-xs text-gray-400 mt-0.5">{NOTIF_ICONS[n.type] || '[msg]'}</span>
...
function Row({ label, value, multiline }) {
  return (
    <div className={multiline ? 'space-y-1' : 'flex justify-between text-sm gap-4'}>
      <span className="text-gray-500 text-sm">{label}</span>
      <span className={`text-gray-800 ${multiline ? 'text-sm whitespace-pre-wrap' : 'font-medium text-right max-w-xs'}`}>{value}</span>
    </div>
  );
}
